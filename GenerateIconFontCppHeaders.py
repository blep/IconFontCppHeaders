...
class FontFA4( Font ):              # legacy Font Awesome version 4
    ...
    def get_icons( self, input ):
        ...
        for item in data[ 'icons' ]:
            ...
            icons.append([ item[ 'id' ], item[ 'unicode' ], item.get('filter', []) or []])
...
class Language:
    ...
    def save_to_file( cls ):
        ...
        if hasattr(cls, 'catalog_file_name'):
            filename = cls.catalog_file_name.format(name=str(cls.intermediate.get('font_name')).replace(' ', ''))
            catalog = cls.geneate_catalog()
            with open(filename, 'wt') as f:
                f.write(catalog)
            print('Saved - {!s}').format(filename)
...
class LanguageCpp11( LanguageC89 ):
    ...
    catalog_file_name = 'IconsDesc{name}.hpp'
...
extern FontIconDesc font_icon_desc_{abbr}[{nb_icon}];
extern FontIconFilter font_icon_filters_{abbr}[{nb_filter}];
extern int16_t font_icons_by_filter_{abbr}[{nb_icons_by_filter}];
extern int16_t font_filters_by_icon_{abbr}[{nb_filters_by_icon}];

#if defined(FONT_ICON_{abbr}_IMPL)
FontIconFilter font_icon_filters_{abbr}[{nb_filter}]= {{
{filters}
}};

FontIconDesc font_icon_desc_{abbr}[{nb_icon}] = {{
'''
        lines =[]
        icons_by_filter = defaultdict(list)
        for icon_index, icon in enumerate(cls.intermediate.get( 'icons' )):
            if len(icon) > 2:
                for filter_name in icon[2]:
                    icons_by_filter[filter_name].append( icon_index)
        all_filters = sorted(icons_by_filter)
        filter_index_by_name = {name: index for index, name in enumerate(all_filters)}
        # for each filter, index of the related icons
        icon_indexes = []
        filters_decl = []
        for filter_name in all_filters:
            filter_icon_indexes = icons_by_filter[filter_name]
            line = '    {"%s", %d, %d}' % (filter_name, len(icon_indexes), len(filter_icon_indexes))
            filters_decl.append(line)
            icon_indexes.extend(filter_icon_indexes)
        # for each icon, indexes of the related filters
        icon_filter_info = []
        filter_indexes = []
        for icon in cls.intermediate.get( 'icons' ):
            if len(icon) > 2:
                icon_filter_indexes = [filter_index_by_name[name] for name in sorted(icon[2])]
            else:
                icon_filter_indexes = []
            icon_filter_info.append((len(filter_indexes), len(icon_filter_indexes)))
            filter_indexes.extend(icon_filter_indexes)
        ...
        result = tmpl_prelude.format(lang = cls.language_name,
                                     ...
                                     font_ttf = cls.intermediate.get( 'font_ttf' ),
                                     abbr=abbr,
                                     nb_icon = len(cls.intermediate.get( 'icons' )),
                                     filters = filters_decl,
                                     nb_filter = len(all_filters),
                                     nb_icons_by_filter = len(icon_indexes),
                                     nb_filters_by_icon = len(filter_indexes))
        lines.append(result)
        for icon_index, icon in enumerate(cls.intermediate.get( 'icons' )):
            tmpl_line_icon = '    {{ "{name}", "ICON_{abbr}_{icon}", u8"\u{code}", {filter_index}, {nb_filter} }},'
            icon_name = str.upper( icon[ 0 ]).replace( '-', '_' )
            icon_code = icon[ 1 ]
            filter_info = icon_filter_info[icon_index]
            line_icon = tmpl_line_icon.format(abbr =abbr,
                                              name = icon[0],
                                              icon = icon_name,
                                              code = icon_code,
                                              filter_index=filter_info[0],
                                              nb_filter=filter_info[1])
            lines.append(line_icon)
        lines.append('};\n')
        lines.extend( cls._generate_index_array(icon_indexes, 'font_icons_by_filter_' + abbr ))
        lines.extend( cls._generate_index_array(filter_indexes, 'font_filters_by_icon_' + abbr ))
        lines.append('''\
...
''')
    ...
    @staticmethod
    def _generate_index_array(indexes, name):
        lines = [ 'int16_t %s[%d] = {' % (name, len(indexes)) ]
        index_per_row = 28
        for start_index in range(0, len(indexes), index_per_row):
            lines.append( '    ' + ','.join(str(i) for i in indexes[start_index:start_index+index_per_row]) + ',')
        lines.append( '};\n' )
        return lines
